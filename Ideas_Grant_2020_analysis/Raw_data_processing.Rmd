...
# Import all VANANZ PI and growth curve data (plate 1 to 6 + P1, P2, M1, M2)
nb experiment PI kinetics = 20
nb experiment growth curves = 15

```{r}

raw_PI.df <- read.csv("Ideas_Grant_2020_analysis/Raw_data/all_cell_death_data_parsed.csv")
unique(raw_PI.df$experiment)
unique(raw_PI.df$plate)
raw_PI.df %>% filter(!duplicated(experiment)) %>% count(plate, .drop = F)

  
raw_GC.df <- read.csv("Ideas_Grant_2020_analysis/Raw_data/all_growth_data.csv")
unique(raw_GC.df$experiment)
...
raw_PI_check <- raw_PI.df %>%
  check_reference(plot = TRUE,
                  outlier_sd_proportion = .05,
                  outlier_tukey_proportion = .05,
                  outlier_mad_proportion = .05,
                  filter = F)
...
write.csv(clean_PI.df, "Ideas_Grant_2020_analysis/Processed_data/processed_PI_kinetics.csv", row.names=FALSE)
write.csv(mean_clean_PI_kinetics.df, "Ideas_Grant_2020_analysis/Processed_data/processed_mean_PI_kinetics.csv", row.names=FALSE)
write.csv(median_clean_PI_kinetics.df, "Ideas_Grant_2020_analysis/Processed_data/processed_median_PI_kinetics.csv", row.names=FALSE)
write.csv(parameters_PI.df, "Ideas_Grant_2020_analysis/Processed_data/processed_parameters_PI.csv", row.names=FALSE)
write.csv(mean_sample_parameters_PI.df, "Ideas_Grant_2020_analysis/Processed_data/processed_mean_parameters_PI.csv", row.names=FALSE)
write.csv(median_sample_parameters_PI.df, "Ideas_Grant_2020_analysis/Processed_data/processed_median_parameters_PI.csv", row.names=FALSE)
```
...
info.df <- read.csv("plate_info/well_info_n843_M1M2_P1P2.csv")
