---
title: 'VANANZ closely related pairs analysis: update with GP plates'
author: "R Guerillot / Stefano Giulieri"
date: "18/08/2020"
...
# confirm number of pairs
close_strain_symetrical_with_pheno_changes_no_dup.df %>%
  filter(switches == "Survived-Died") %>%
  distinct(iso1, iso2) %>%
  nrow()

close_strain_symetrical.df %>%
  filter(iso1_mortality == "Survived" & iso2_mortality == "Died") %>%
  distinct(iso1, iso2) %>%
  nrow()

# number of strains
n_distinct(c(close_strain_symetrical_with_pheno_changes_no_dup.df$iso1, close_strain_symetrical_with_pheno_changes_no_dup.df$iso2))
