...
# parse results
PI_A1 <- parse_kinetics(file = file, duration = duration, wavelength = wavelength, exp = exp, cell_number = cell_number, plate = plate, plot_data = F, n_skip = 14)
```
...
```{r}
raw_PI.df <- cell_death_raw_df %>%
  mutate(sample_id = ifelse(strain_group %in% c("SSR-ST5","SSR-ST8"), yes = as.character(strain_description), no = sample_id))
```

# save raw data
```{r}
#write.csv(raw_PI.df, "Ideas_Grant_2020_analysis/Raw_data/NEB_all_cell_death_raw_data_parsed.csv", row.names = F)
```
...
# Identify the NEBRASKA mutants screened that are in genetic pairs analysis
```{r}
genetic_pairs_conv.df <- read.csv("./Ideas_Grant_2020_analysis/Convergence_analysis_tables/convergence_analysis_clusters_no_reduntant_controls.csv")

genetic_pairs_conv_analysed.df <- genetic_pairs_conv.df %>%
  filter(neb_mutant_id %in% cell_death_raw_df$sample_id) %>%
...
# And Plot
```{r}
GRANT_NEB.df <- clean_PI.df %>%
  filter(sample_id != "Maximum cell death") %>%
  filter(strain_group == "CONTROL" | sample_id %in% genetic_pairs_conv_analysed.df$neb_mutant_id) 

plot_cell_death(GRANT_NEB.df, fitted =T, comparator = "JE2", all_replicates = F, ncol =6)
```
