...
plate_planner <- function(isolates, # character vector of samples/isolates names
                          n_replicates = 3,
                          controls, # chacter vector of control names
...
parse_growth <- function(file,
  ...
  data <- read_xlsx(file, skip = n_skip, col_names = col_names) %>%
    ...
    mutate(well = str_c(well_row, 
           ...
           well_id = paste(experiment, plate, well, sep = "_")) %>% # added a variable that identify unique experimental reading) 
    mutate(timepoint = as.numeric(time)) %>%
    select(-c(well_row, well_col, sample, time)) 
...
annotate_wells <- function(df, # dataframe to annotate
  ...
  well_info <- read_csv(well_info) %>%
    transmute(sample_id, well, plate = as.character(plate_number)) %>%
...
sync_experiments <- function(df){
  ...
  # add mean of JE2 max death rate for each experiment to df for synchronisation
  t1 <- t %>%
    ...
    ungroup() %>%
    select(experiment, JE2_point_of_max_death_rate) %>%
    merge(t, ., by = "experiment")
  
  # calculate and apply JE2 time offset for each experiment using JE2 point of max death rate
  t2 <- t1 %>%
...
JE2_POMS_standardisation <- function(df, cut = T, plot = F){
  ...
  JE2_count_df <- df %>% 
    group_by(experiment, sample_id, replicate) %>% 
    filter(row_number() == 1) %>% 
    group_by(experiment, sample_id, .drop = F) %>%
    count(.drop = F) %>%
    filter(sample_id == "JE2") 
  print.data.frame(JE2_count_df)
  
  # Check if number of JE2 replicates in each experiment, 
  # if 0 replicate => experiment is excluded
  # if 1 replicate => raise a warning
  zero_rep_JE2_experiments <- JE2_count_df %>% filter(n == 0) %>% .$experiment
  one_rep_JE2_experiments <- JE2_count_df %>% filter(n == 1) %>% .$experiment
  if (length(zero_rep_JE2_experiments) != 0){
    ...
    df <- df %>% filter(!(experiment %in% zero_rep_JE2_experiments))
  }
...
standardise_curves <- function(df, cut = T, plot = F){ 
  ...
  # if 0 replicate => experiment is excluded
  # if 1 replicate => raise a warning
  zero_rep_JE2_experiments <- JE2_count_df %>% filter(n == 0) %>% .$experiment %>% as.character()
  one_rep_JE2_experiments <- JE2_count_df %>% filter(n == 1) %>% .$experiment %>% as.character()
  if (length(zero_rep_JE2_experiments) != 0){
    warning("The following experiment(s) don't have any JE2 replicate and will be excluded: ", noBreaks. = F)
  ...
  
{
  ...
  outlier_curve.df <- df_out %>%
    ...
    filter(!duplicated(well_id)) %>%
    select(well_id, sample_id, experiment, plate) %>%
    arrange(sample_id, experiment, plate,  well_id)
...
check_reference <- function(df,
                            ...
                            outlier_sd_proportion = 0.1,      # Set the proportion of oultier timepoint in a curve (well_id) to consider outlier
                            outlier_mad_proportion = 0.1,
                            outlier_tukey_proportion = 0.1,
...
fit_smooth_spline <- function(df, spar = 0.8, growth_curves = F){
  ...
  if(!("f_signal.std" %in% colnames(df))) {
    ...
    df_fit <- df %>% 
      group_by(well_id) %>% 
      do(fit = smooth.spline(x = .$timepoint, y = .$f_signal, spar = spar, keep.data = T)) %>%
      augment(fit) %>%
      ungroup() %>%
    ...
    df_fit <- df %>% 
      group_by(well_id) %>% 
      do(fit = smooth.spline(x = .$timepoint, y = .$f_signal.std, spar = spar, keep.data = T)) %>% 
      # run smooth.spline and store the resulting list in a variable fit
      augment(fit) %>% # add components of the list as new variables
      ungroup() %>%
...
plot_cell_death <- function(df, 
                            ...
                            combine_experiments = "all", # can set to any string corresponding to a column/variable, "all" to plot the combined results from different experiments, "plate" to combine results from different experiments for the same plate
                            annotate = F, # annotate well_id on plot (eg. to identify well_id of outliers)
                            comparator = F, # add a comparator strain to each facet (set comparator to a sample_id)
                            ...
                            return_plots = F, # return list of plots if set to TRUE
  ...
  #combine_experiments <- match.arg(combine_experiments) # use match.arg() to extract the default ("none") if no value is given
  
  if (combine_experiments == "all") {
    df$experiment <- "Combined"
    #} else if (combine_experiments == "plate") {
    #  df$experiment <- df$plate
  } else {
    df$experiment <- df[,c(combine_experiments)]
  }
  ...
  for (exp in unique(df$experiment)) {
    ...
    df_exp <- filter(df, experiment == exp)
...
parse_growth <- function(file,
                         n_skip = 14,
                         well_cols = c("separated", "merged"), # indicate whether well columns are separated in row-col (default) or merged
                         interval = 15,
                         duration,
                         normalised = FALSE,
                         exp,
...
plot_growth_curves <- function(df, 
                               standardized = F, # set to TRUE to plot stadardised data (need to run JE2_POMS_standardisation first)
                               sync = F, # set to TRUE to synchronised timepoints according to JE2 max tox rate (need to run JE2_sync first)
                               fitted = F, # set to TRUE to plot growth infered from smooth spline models (need to run fit_smooth_spline first)
                               residuals = F,  # set to TRUE to plot residuals from smooth spline models (need to run fit_smooth_spline first)
                               all_replicates = 15, # set to FALSE if you only want mean + standard deviation and not all replicate points/curves can be set to a numeric (eg. 24 will plot value every 24 min only)
                               combine_experiments = c("none", "all", "plate"), # set to "all" to plot the combined results from different experiments, "plate" to combine results from different experiments for the same plate
                               annotate = F, # annotate well_id on plot (to identify outliers)
                               comparator = F, # add a comparator strain to each facet (set comparator to a sample_id)
                               filter_outliers = F, 
                               derivative1 = F, # plot 1st derivatives
                               derivative2 = F, # plot 2nd derivatives
  ...
  timepoints <- seq(0, timepoint_max, interval) 
  col_names_OD <- str_c("OD_", timepoints)
  if (well_cols == "separated") {
    colnames_well <- c("well_row", "well_col")
  } else {
    colnames_well <- "well"
  }
  col_names <- c(colnames_well, "sample", col_names_OD)
...
annotate_wells <- function(df, # dataframe to annotate
                           well_info = "plate_info/well_info_n843.csv", # dataframe mapping strains to wells
                           metadata = "plate_info/strain_metadata.csv", # metadata
                           type = c("minimal", "wide")) {
  ...
  controls <- c("JE2", "TOX-4", "Complete lysis", "Non infected")
  well_info <- read_csv(well_info) %>%
    transmute(sample_id, replicate, well, plate = as.character(plate_number)) %>%
    mutate(
      sample_id = if_else(sample_id == "TOX-5", "JE2", sample_id),
      sample_id = if_else(sample_id == "Total LDH", "Complete lysis", sample_id)
    ) 
  ...
  # retrieve metadata
  strain_info <- read_csv(file = metadata) 
  
  if (derivative1) {
    df <- df %>%
      mutate(OD = fitted.deriv1)
    type = "fitted 1st derivative" 
  }
  
  if (derivative2) {
    df <- df %>%
      mutate(OD = fitted.deriv2)
    type = "fitted 2nd derivative" 
  }
