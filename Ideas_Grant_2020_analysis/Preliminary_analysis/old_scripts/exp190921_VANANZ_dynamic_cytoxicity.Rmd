---
title: "VANANZ dynamic cytotoxicity experiment 09/09/2019"
...
#file <- "/Raw_data/exp_190921/PI_kinetics/190921 HeLa real time cell death assay n4 using 20000 cells .xlsx"
#dir  <- getwd()
#path <- str_c(
#  dir,
#  file
#)
n_skip = 12 # set number of lines to skip in the Excel sheet
timepoint_interval <- 0.1
# generates vector of timepoints (in minutes) of fluorescence measurement. Here: every 6 minutes until 2 hours
timepoint_max <- 20.7 *2
timepoints <- seq(0, timepoint_max, timepoint_interval) * 60 
# generate vector of column names based on timepoints and wavelength. Here, only wave length 535 was used
col_names_WL <- str_c("WL535_", timepoints)
col_names <- c(
  "well",
  "sample",
  col_names_WL
)
data <- read_excel(path = "../Raw_data/exp_190921/PI_kinetics/190921 HeLa real time cell death assay n4 using 20000 cells .xlsx", col_names = col_names, skip = n_skip) 
...
```{r}
file <- "../Raw_data/Preliminary_analysis/Raw_data/exp_190909/PI_endpoints/190910.plate.Vanaz2.HeLa.20000.cell.death.rate.dynamic.run.3.endpoint.xl"
path <- str_c(
  dir,
  file
)
...
## Plot 535 WL with PI at end point
```{r}
data_long %>%
  ggplot(aes(x = timepoint, y = f_signal, color = wave_length)) +
  geom_point(size = .5) +
...
path <- str_c(dir, file)
df %>%
  mutate(exp = "exp190909") %>%
  write_csv(path = path)

# Summary dataframe
file <- "/Preliminary_analysis/Analysis/dataframes/exp190909_df_summary.csv"
path <- str_c(dir, file)
df_summary %>%
  mutate(exp = "exp190909") %>%
  write_csv(path = path)
