...
plate_planner <- function(isolates, # character vector of samples/isolates names
                          ...
                          n_replicates_controls = 3, # numeric with number of replicates for the controls. If not default (3), must have same length and order like the controls vector 
  ...
  # Generate sample/control vector when all have 3 replicates
  if (length(n_replicates_controls) == 1){
    if (n_replicates_controls == 3){
    ...
    # Generate vector for controls
    control_samples <- c()
    for (i in 1:length(controls)){
      my_sample <- controls[i]
      my_replicate <- n_replicates_controls[i]
      
      control_samples <- c(control_samples, rep(my_sample, my_replicate))
      
    }
    
    sample_id <- c(sample_id, control_samples)
  ...
  replicates_df <- dplyr::tibble(sample_id, replicate) %>%
    replace_na(list(sample_id = "Non infected"))
