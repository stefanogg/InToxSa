...
setwd(stringr::str_c(here::here(), "/InToxSa_paper/Mutagenesis"))
...
```{r}
PI_kinetics <- readRDS("../../Mutants_2022/processed_data/PI_kinetics/PI_kinetics_plateGP-C1.Rda")
```

PI parameters plate GP-C1

```{r}
PI_parameters <- readRDS("../../Mutants_2022/processed_data/PI_parameters/PI_parameters_plateGP-C1.Rda") %>%
  as_tibble()
```
...
df_plot %>%
  ...
  geom_point() +
  stat_summary(fun.data = "mean_sd", geom = "crossbar", width = .5) +
  facet_wrap(~gene, scales = "free_x") +
  theme_bw()
...
df_plot %>%
  ...
  stat_summary(fun.data = "mean_sd", geom = "crossbar", width = .5, position = "dodge") +
  facet_wrap(~gene, scales = "free_x") +
  theme_bw()
