...
```{r}
PI_parameters <- readRDS("../../5-Data_processing/processed_data/clinical_strains/PI_parameters/PI_parameters.Rda") %>%
  as_tibble() %>%
  select(sample_id, pi_auc = AUC_death)

PI_mean <- readRDS("../../5-Data_processing/processed_data/clinical_strains/PI_parameters/PI_sample_parameters.Rda") %>%
  as_tibble() %>%
...
```{r}
PI_pairs <- readRDS("raw_data/df_cleaned_pair_PI.Rda")
```
