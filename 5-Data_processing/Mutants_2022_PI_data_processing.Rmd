...
```{r message=FALSE}
df_PI_raw <- list.files(path="../3-Data_parsing/parsed_experiments/", pattern="PI_data_plateGP-C1", full.names=TRUE, recursive=T) %>% 
...
Overall, a very low proportion of outliers and no sample excluded! Good work!
...
df_PI_clean %>%
  saveRDS("processed_data/allelic_exchange/PI_kinetics/PI_kinetics_plateGP-C1.Rda", compress = "gzip")
```
...
df_PI_clean_parameters %>%
  saveRDS("processed_data/allelic_exchange/PI_parameters/PI_parameters_plateGP-C1.Rda")

df_PI_clean_parameters_sample %>%
  saveRDS("processed_data/allelic_exchange/PI_parameters/PI_sample_parameters_plateGP-C1.Rda")
```