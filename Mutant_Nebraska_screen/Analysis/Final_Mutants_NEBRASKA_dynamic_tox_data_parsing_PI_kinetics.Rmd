...
# standardise with JE2, fit curve and detect outliers
```{r}
cell_death_df <- cell_death_raw_df %>%
  JE2_POMS_standardisation(., cut = T) %>%
  fit_smooth_spline(.) %>%
  flag_outlier_timepoints(k_sd = 1.5)
```

# get outliers summaries
```{r}
cell_death_outliers_df <-summarise_outlier(cell_death_df, k_sd = 1.5)
```

# plot outliers (need to turn into a function)
```{r}
t <- cell_death_outliers_df

ggplot(t, aes(x=well, y = proportion_outlier_mad, fill = experiment)) +
  geom_bar(stat = "identity") +
  facet_wrap(~experiment, ncol = 1)

ggplot(t, aes(x=well, y = proportion_outlier_sd, fill = experiment)) +
  geom_bar(stat = "identity") +
  facet_wrap(~experiment, ncol = 1)

ggplot(t, aes(x = experiment, y = proportion_outlier_mad)) +
  geom_boxplot() +
  geom_point(aes(colour = sample_id == "JE2"))

ggplot(t, aes(x = well, y = proportion_outlier_mad)) +
  geom_boxplot() +
  geom_point(aes(colour = sample_id == "JE2"))

ggplot(t %>% filter(proportion_outlier_mad > 0.2), aes(x = well)) +
  geom_histogram(stat = "count") +
  coord_flip()

```

# filter outliers and restandardise data
```{r}

outliers <- cell_death_outliers_df %>% 
  filter(proportion_outlier_sd >= 0.20) %>%
  .$well_id

outliers_JE2 <-  cell_death_outliers_df %>% 
  filter(sample_id == "JE2") %>%
  filter(proportion_outlier_sd >= 0.10) %>%
  .$well_id

outliers_all <- c(outliers, outliers_JE2)

cell_death_df_2 <- cell_death_raw_df %>%
  filter(!(well_id %in% outliers_all)) %>%
  JE2_POMS_standardisation(., cut = T) %>%
  fit_smooth_spline(.) %>%
  flag_outlier_timepoints(.)

# summarise outliers
cell_death_outliers_df_2 <- summarise_outlier(cell_death_df_2)

# plot outliers
t <- cell_death_outliers_df_2
...
ggplot(t, aes(x = experiment, y = proportion_outlier_sd)) +
  geom_boxplot() +
  geom_point(aes(colour = sample_id == "JE2"))

ggplot(t, aes(x = well, y = proportion_outlier_sd)) +
  geom_boxplot() +
  geom_point(aes(colour = sample_id == "JE2"))
...
NEBRASKA_df <- cell_death_df_2 %>%
  filter(sample_id != "Maximum cell death") %>%
  filter(strain_group == "NEBRASKA" | strain_group == "CONTROL") 
plot_cell_death(df = NEBRASKA_df, fitted = T, combine_experiments = "all", comparator = "JE2", all_replicates = 24, annotate = T)
```
