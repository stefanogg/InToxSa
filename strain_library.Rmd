...
data <- read_csv("~/Dropbox/VANESSA_ANZCOSS_R/dataframes/VAN_VSS_n843.csv")
```
...
We will add a maximum of 28 isolates per plate (84 replicates). The remaining 12 wells will be used for non infected cells (3 wells), low toxic control (tox 4 = BPH3757, 3 wells), high toxic control (tox 5 = BPH2705, 3 wells), and for total LDH determination (using Triton, 3 wells).
...
write_csv(
  sample_list_plate,
  "plate_info/sample_list_plate.csv"
)
```
...
plate_info_1
write_csv(plate_info_1, "plate_info/plate_info_1.csv")
```
...
```{r warning=FALSE}
dir.create("plate_info/plate_maps")
