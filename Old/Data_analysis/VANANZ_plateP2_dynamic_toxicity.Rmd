...
file <- "Raw_data/plateP2/exp200226/PI_kinetics/200226 Plate P2 n1 HeLa 40000 cell death assay 44h.xlsx"
duration <- c(43, 36)
exp <- "exp200226"
cell_number <- 40e3
plate <- "P2"
...
PI_kinetics_data %>%
  write_csv("Data_parsing/dataframes/partial_datasets/plateP2_PI_kinetics_well_info.csv")
...
```{r}
col_names <- c("well", "sample_id", "replicate", "cfu_pre_dilution2", "cfu_pre_dilution3",
               "cfu_post_dilution0", "cfu_post_dilution1", "cfu_post_dilution2")
cfu_data <- readxl::read_xlsx(path = "Raw_data/plateP2/exp200226/CFUs/Plate_P2_trial_1_27-2-20.xlsx", sheet = 6, skip = 1,
                      col_names = col_names) 
...
#
spot_vol <- 5 # spot volume in ul
df_cfu_summary <- df_cfu %>%
  group_by(sample_id, pre_infection, count_type) %>%
  mutate(n_values = length(which(!is.na(cfu_count)))) %>%
  group_by(sample_id, pre_infection) %>%
  mutate(max_n_values = max(n_values)) %>%
  group_by(sample_id, pre_infection, count_type) %>%
  mutate(all_values = length(which(!is.na(cfu_count))) == max_n_values) %>%
  filter(all_values) %>%
  group_by(sample_id, pre_infection) %>%
...
df_cfu_summary %>%
  ...
  geom_boxplot() +
  scale_y_continuous(trans = "log10") +
  facet_wrap(~strain_group, scales = "free_x", ncol = 5) +
  labs(y = "Fold change (post-infection / pre-infection\n", x = "") +
  theme_bw() +
  theme(legend.position = "none")

ggsave("Data_analysis/figures/CFUs/cfu_fold_change_plate_P2.pdf", width = 11, height = 5)


df <- df_cfu_summary %>%
  mutate(fold_change = log10(fold_change))
for (i in unique(df$strain_group)){
  p <- ggboxplot(data = df %>% filter(strain_group == i),
                 y = "fold_change",
                 x = "sample_id") +
...
df <- compare_means(fold_change ~ sample_id, df_parameters, method = "t.test", group.by = "strain_group")
