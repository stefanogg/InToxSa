...
knitr::opts_knit$set(root.dir = "~/Documents/Travail/Abdou_project/Staph_infection_project/github_analysis/VANANZ_phenotypes")
```
...
# Select NE896 strains
```{r}
library(ggsci)

cell_death_df <- cell_death_raw_df %>%
  filter(plate %in% c("D","F","G")) %>%
  filter(sample_id %in% c("NE896", "JE2")) %>%
  standardise_curves() %>%
  fit_curves()

# p <- plot_cell_death(cell_death_df, fitted = T, combine_experiments = "plate", comparator = "JE2", title = "", return_plots = T)
# print(cowplot::plot_grid(plotlist = p))
# fill = "#045a8d"
# color = "#045a8d"
 p1 <- cell_death_df %>%
      ggplot(aes(x = timepoint/60, y = f_signal.fitted, fill = sample_id, colour = sample_id)) +
      #stat_summary(geom = "ribbon", fun.data = mean_cl_normal, fun.args=list(conf.int=0.95), alpha = 0.2) + # Manually checked and give same results, but 95% CI appear to be too narrow 
      #stat_summary(geom = 'ribbon', fun.data = 'mean_sdl', fun.args = list(mult = 1), alpha = 0.1) + # Use standard deviation instead
...
p2 <- cell_death_df %>%
      ...
      labs(title = "",
           x = "Time post-infection (hour)",
           y = "Cell death - Fluorescence intensity (A.U.)") +
      theme_bw() +
  scale_color_nejm() +
  scale_fill_nejm()
p2
