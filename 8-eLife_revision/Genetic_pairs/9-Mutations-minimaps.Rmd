...
df_plot %>%
  ggplot(aes(x = AA_POS, label = MUTATION_SHORT, y = .5)) +
...
Plot multipanel. Note that we want to highlight mutations that were reconstructed

```{r}
allexc <- c("E7K", "G310fs", "K2308fs")
df_plot <- df_genes %>%
  ungroup() %>%
  ...
  mutate(pair_number = min(row),
         text_size = MUTATION_SHORT %in% allexc) 
...
p1 <- df_plot %>%
  ggplot(aes(x = AA_POS, label = MUTATION_SHORT, y = .5,  fill = EFFTYPE_SHORT, size = text_size)) +
  geom_segment(aes(x = AA_POS, xend = AA_POS, y = 0, yend = .5),  size = 1, colour = "black") +
  ...
  scale_y_continuous(limits = c(0, 20), expand = c(0,0), breaks =  NULL) +
  scale_size_discrete(guide = "none", range = c(4,6)) +
