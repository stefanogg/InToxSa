...
Here we try a different approach to the genetic pairs analysis. Instead of looking at convergent within pairs with a definite phenotype, we look at phenotical differences between independent pairs *with* or *without* homoplasic mutations
...
df <- df_complete %>%
  mutate(delta_auc = abs(delta_auc)) %>%
 select(-c(is_truncated, pair_id, PI_pair, replicate)) %>%
  group_by(neb_gene_symbol, is_mutated) %>%
...
(p1 <- df_auc %>%
  ...
  distinct() %>%
  ggplot(aes(x = fct_rev(gene_level), y = median_delta, fill = is_mutated)) +
