...
Following the eLife reviews, we add the position of the transposon insertion in the Nebraska library
...
```{r}
df_mutations <- readRDS("processed_data/gene_convergence/PI_discordant/df_PI_convergent_genes.Rda")
```
...
df_tn_insert <- tn_insert_data %>%
  ...
  mutate(AA_POS = if_else(neb_strand == "+",
                          ceiling((tn_insert - neb_start)/3),
                          ceiling((neb_end - tn_insert)/3)))
...
#   saveRDS("processed_data/gene_convergence/PI_discordant/convergence_plots.Rda")
```
