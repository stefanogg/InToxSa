...
```{r}
files <- list.files("~/Documents/Transfer_with_server/", pattern = "pyseer-nodist", full.names = T)
...
```{r}
files <- list.files(dir, recursive = T, full.names = T, pattern = "pyseer-nodist")
df_files <- tibble(path = files,
                   dataset = "Genetic pairs",
                   phenotype = str_remove(basename(files), ".pyseer-nodist.tab"),
...
(p7 <- df_plot %>%
  # filter(rare_mutations == "TRUE" & truncations == "FALSE") %>%
  ggplot(aes(x = start/1e6, y = -log10(p_value))) +
  geom_point(aes(colour = beta < 0, size = af), alpha = .3) +
  geom_text_repel(aes(label = label_gene_symbol), fontface = "bold") +
