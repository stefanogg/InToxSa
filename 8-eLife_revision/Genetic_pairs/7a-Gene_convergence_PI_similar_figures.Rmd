---
title: 'Generate figures for the convergent evolution analysis: pairs with no / minimal PI change'
...
Here we generate figure for the gene enrichment analysis. We focus on pairs with no /minimal PI difference. The whole gene convergence analysis process is described in `4b-Summarise pairs_info_with_no_PI_change.Rmd`.
...
```{r}
df_mutations <- readRDS("processed_data/gene_convergence/PI_similar/df_PI_convergent_genes_eqPI.Rda")
```
...
(p1 <- df_plot %>%
  ggplot(aes(x = fct_reorder(neb_gene_symbol, cluster_order, .desc = T),
             y = n_mutations,
             fill = factor(CC_iso1, levels = names(colours)))) +
  geom_bar(stat = "identity") +
...
```{r}
# list(p1, p2) %>%
#   saveRDS("processed_data/gene_convergence/PI_similar/convergence_plots_eqPI.Rda")
```
