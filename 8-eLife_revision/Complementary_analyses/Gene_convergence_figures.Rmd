...
setwd(stringr::str_c("~/Documents/Github/InToxSa", "/8-eLife_revision"))
...
```{r}
df_mutations <- readRDS("../6-InToxSa_paper/Genetic_pairs/processed_data/gene_convergence/df_PI_convergent_genes.Rda")
...
```{r}
df_PI_NEB <- readRDS("../5-Data_processing/processed_data/neb_mutants/PI_parameters/PI_sample_parameters_plateGPV1.Rda")

df_PI_NEB_wells <- readRDS("../5-Data_processing/processed_data/neb_mutants/PI_parameters/PI_parameters_plateGPV1.Rda")
```
...
```
Add position of the transposon insertion in the Nebraska library

```{r}
tn_insert_data <- read_csv("raw_data/neb_tn_insertions.csv") %>%
  select(sample_id = StrID, tn_insert = Pos, tn_direction = Tr)

df_neb <- read_csv("../6-InToxSa_paper/Genetic_pairs/raw_data/df_FPR3757_curated.csv")

df_tn_insert <- tn_insert_data %>%
  inner_join(df_neb, by = c("sample_id" = "neb_mutant_id")) %>%
  mutate(AA_POS = ceiling((tn_insert - neb_start)/3))

df_gene_maps2 <- df_tn_insert %>%
  inner_join(cluster_order) %>%
  transmute(cluster_order, neb_gene_symbol, AA_POS, EFFTYPE_SHORT = "IS insertion (intragenic)",
         tn_insertion = T)

df_gene_maps <- df_gene_maps %>%
  mutate(tn_insertion = F) %>%
  bind_rows(df_gene_maps2)
...
p2 <- df_gene_maps %>%
  ggplot(aes(x = cluster_order, y = AA_POS, colour = EFFTYPE_SHORT, shape = tn_insertion, size = as.integer(tn_insertion))) +
  geom_point(fill = "#542788") +
  ...
  scale_color_manual(values = rev(colors), na.translate = F, guide = F) +
   scale_shape_manual(values = c(16, 25), na.translate = F, guide = F) +
  scale_size_continuous(guide = F, range = c(2,6) ) +
...
df_neb_stats <- df_PI_NEB %>%
  ...
  left_join(df_mutations %>% distinct(neb_gene_symbol, neb_mutant_id),
...
```{r}
Cell_count.df <- readRDS("../6-InToxSa_paper/Genetic_pairs/raw_data/Cell_count_corrected.df.Rda") %>%
  mutate(cell_id = str_c(sample_id, plate_number, plate_replicate, replicate, timepoint, well, Field, Object_No, sep = "_"))

Spot_Cell_count.df <- readRDS("../6-InToxSa_paper/Genetic_pairs/raw_data/Spot_Cell_count.df.Rda")
...
#   saveRDS("processed_data/convergence_plots.Rda")
```
